...
def test_create_and_delete_post(
    api_client, get_auth_headers, create_fake_post, monkeypatch
):
...
@pytest.mark.django_db
def test_patch_post(api_client, get_auth_headers, create_fake_post, monkeypatch):
    post_id = create_fake_post
    ...
    response = api_client.patch(
        f"/post/{post_id}", headers=get_auth_headers, data=to_update
    )
